...
io.on('connection', (socket) => {
    ...
    socket.on('startGame', (roomId) => {
        const room = rooms[roomId];
        if (!room) return;
        if (room.creatorId !== socket.id) return socket.emit('errorMsg', 'តែអ្នកបង្កើតបន្ទប់ប៉ុណ្ណោះអាចចុច Start!');
        if (room.status !== 'waiting') return socket.emit('errorMsg', 'ហ្គេមបានចាប់ផ្តើមហើយ!');
        if (room.players.length < 2) return socket.emit('errorMsg', 'ត្រូវការអ្នកលេងយ៉ាងហោចណាស់ ២ នាក់!');
        startGameNow(roomId);
    });
    ...
    function startGameNow(roomId) {
        ...
        room.players.forEach((p, i) => {
            ...
            if (specs.length > 0) specialAlerts.push({ player: p.name, hands: specs });
        });
    ...
    socket.on('playCard', ({ roomId, cardValue, cardSuit }) => {
        ...
        if (!player) return;
        if (room.players.indexOf(player) !== room.currentTurnIndex) return socket.emit('errorMsg', 'មិនមែនវេនរបស់អ្នកទេ!');
    ...
    socket.on('disconnect', () => {
        for (let id in rooms) {
            ...
            if (pIdx !== -1) {
                ...
                if (room.status === 'playing') io.to(id).emit('gameStatus', '⚠️ មានអ្នកលេងបានចាកចេញ!');
                break;
            }
        }
    });
