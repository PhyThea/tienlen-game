...
const io = new Server(server, {
    maxHttpBufferSize: 1e8, // ការពារការដាច់កញ្ចប់ទិន្នន័យសំឡេងធំៗ
    cors: { origin: "*" }
});
...
function moveToNextTurn(room) {
    ...
    for (let i = 1; i <= room.players.length; i++) {
        ...
        if (p && p.hand.length > 0 && !p.passed) {
            room.currentTurnIndex = checkIndex;
            found = true;
            break;
        }
    }
...
io.on('connection', (socket) => {
    ...
    // 🎙️ បាញ់ចែកចាយសំឡេង Raw PCM កម្រិតល្បឿនលឿន (Realtime Buffer)
    ...
    socket.on('playCards', ({ roomId, cards }) => {
        ...
        if (room.players[playerIdx].id !== socket.id) return socket.emit('errorMsg', 'មិនមែនជាវេនរបស់អ្នកទេ!');
        ...
        if (player.hand.length === 0) {
            ...
            if (room.players.filter(p => p.hand.length > 0).length <= 1) {
                room.status = 'waiting';
                io.to(roomId).emit('gameOver', { winner: player.name, ranks: room.players.map(p => ({ id: p.id, name: p.name, rank: p.rank || room.nextRank, hand: p.hand, cardsLeft: p.hand.length })) });
                return;
            }
        }
