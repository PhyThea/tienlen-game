...
function checkSpecialHands(hand) {
    ...
    if(maxStreak >= 5) specials.push(`សល់ហាយ (${maxStreak} សន្លឹក)`);

    if(hand.some(c => c.value === '3' && c.suit === '♣')) {
        specials.push(`មាន 3♣`);
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('createRoom', ({ roomId, password, playerName }) => {
        ...
        io.to(roomId).emit('gameStatus', `⏳ រង់ចាំអ្នកលេង... (យ៉ាងហោច ២ នាក់)`);
    ...
    socket.on('joinRoom', ({ roomId, password, playerName }) => {
        ...
        io.to(roomId).emit('gameStatus', `👤 ${newPlayer.name} បានចូលបន្ទប់...`);
    ...
    socket.on('startGame', (roomId) => {
        ...
        if (room.creatorId !== socket.id) {
            return socket.emit('errorMsg', 'តែអ្នកបង្កើតបន្ទប់ប៉ុណ្ណោះអាចចុច Start!');
        }
        if (room.status !== 'waiting') {
            return socket.emit('errorMsg', 'ហ្គេមបានចាប់ផ្តើមហើយ!');
        }
        if (room.players.length < 2) {
            return socket.emit('errorMsg', 'ត្រូវការអ្នកលេងយ៉ាងហោចណាស់ ២ នាក់!');
        }
        
        startGameNow(roomId);
    });

    function startGameNow(roomId) {
        const room = rooms[roomId];
        ...
        room.players.forEach((p, i) => {
            ...
            const specs = checkSpecialHands(p.hand);
            if (specs.length > 0) {
                specialAlerts.push({ player: p.name, hands: specs });
                console.log(`🔥 ${p.name} has special: ${specs.join(', ')}`);
            }
        });
        ...
        io.to(roomId).emit('gameStatus', `🎮 ហ្គេមចាប់ផ្តើម! វេន: ${room.players[room.currentTurnIndex].name}`);
    ...
    socket.on('playCard', ({ roomId, cardValue, cardSuit }) => {
        ...
        if (room.players.indexOf(player) !== room.currentTurnIndex) {
            return socket.emit('errorMsg', 'មិនមែនវេនរបស់អ្នកទេ!');
        }
        ...
        room.playedCards.push({ ...playedCard, by: player.name });

        console.log(`🃏 ${player.name} played ${playedCard.value}${playedCard.suit}`);
        ...
        for (let id in rooms) {
            ...
            if (pIdx !== -1) {
                ...
                if (room.status === 'playing') {
                    io.to(id).emit('gameStatus', '⚠️ មានអ្នកលេងបានចាកចេញ!');
                }
                break;
            }
        }
...
server.listen(PORT, () => {
    ...
    console.log('🃏 Ready to play Tien Len!');
});